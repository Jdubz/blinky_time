...
// Permutation table
const uint8_t SimplexNoise::permTable[256] = {
  ...
  92,41,55,46,245,40,244,102,143,54, 65,25,63,
  161, 1,216,80,73,209,76,132,187,208,89,18,169,
  ...
  119,248,152, 2,44,154,163, 70,221,153,101,
  ...
  114,67,29,24,72,243,141,128,195,78,66,215,61,
  156,180
};
...
float SimplexNoise::noise(float x) {
  // Just use 2D noise with y=0
  return noise(x, 0.0f);
}

float SimplexNoise::noise(float x, float y) {
  // Perlin-style 2D noise (simplified)
  ...
  float res = lerp(
    lerp(grad(perm[A], x, y, 0.0),
         grad(perm[B], x - 1, y, 0.0), u),
    lerp(grad(perm[A + 1], x, y - 1, 0.0),
         grad(perm[B + 1], x - 1, y - 1, 0.0), u),
    v);
  return res; // Range: [-1, 1]
}

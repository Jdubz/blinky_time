...
export class MusicModeRunner extends EventEmitter {
     ...
     */
    async runPattern(patternId, expectedBpm) {
     ...
     */
    analyzeResults(pattern, durationMs, expectedBpm, samples) {
        if (samples.length === 0) {
            return {
                pattern,
                durationMs,
 ...
 */
export async function runMusicModeSweep(options, parameter, values, patterns) {
 ...
 */
export function findOptimalMusicModeValue(results, optimizeFor = 'bpm_accuracy') {
    if (results.length === 0) {
 ...
 */
export function printMusicModeSweepResults(parameter, results) {
    console.log('\n' + '─'.repeat(70));
    ...
    for (const result of results) {
        ...
        console.log(`${String(result.paramValue).padStart(8)} | ${bpmErr} | ${phaseSt} | ${lockT} | ${activePct}`);
    }
    console.log('─'.repeat(70));
}

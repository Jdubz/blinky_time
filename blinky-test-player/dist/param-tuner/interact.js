...
function generateGridCombinations(grid) {
    if (grid.length === 0)
        return [[]];
    if (grid.length === 1)
...
export async function runInteractions(options, stateManager) {
    console.log('\n🔗 Phase 3: Parameter Interactions');
...
function updateOptimalFromInteraction(stateManager, mode, result) {
...
export async function showInteractionSummary(stateManager) {
    console.log('\n🔗 Interaction Summary');
    console.log('═'.repeat(50));
    for (const test of INTERACTION_TESTS) {

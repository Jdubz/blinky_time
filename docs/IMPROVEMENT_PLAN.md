...
> **2026-04-29 update:** v33 (Conv1D W16, 50 mel, fmin=30, fmax=8000) is the deployed onset model on b153 — F1=0.570 on edm/ (hand-curated `.onsets_consensus.json` GT, 18 tracks). **v34d is in training** (kicked off 2026-04-29 11:41) on stem-vs-mix-cleaned `kick_weighted_clean` labels — single-axis change vs v33, only the label source changes. See `docs/ML_IMPROVEMENT_PLAN.md` "active run" header for the full hypothesis, gates, and falsifiable predictions. The previous edm_holdout adversarial corpus was retired 2026-04-27; only `blinky-test-player/music/edm/` (18 tracks) is the validation corpus going forward (rule in CLAUDE.md). All beat-tracker label inputs were archived 2026-04-29 — the task is onset detection, not beat detection (rule also in CLAUDE.md).
>
> **Older "Current Status" content below this callout (firmware b127, v22-v25 model history, ESP32-S3 cut, mel filterbank fix, v26-v32 progression) is historical context preserved for the trail of how we got here. Treat the callout above as the source of truth for current deployment state.**

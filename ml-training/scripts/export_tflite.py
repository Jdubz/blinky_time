...
def tflite_to_c_header(tflite_bytes: bytes, c_array_name: str, output_path: str,
    ...
    # Derive guard from output filename (not c_array_name) to avoid
    # duplicate guards when versioned headers coexist (e.g., _v16.h, _v17.h)
    stem = Path(output_path).stem.upper().replace("-", "_").replace(".", "_")
    guard_name = stem + "_H"
...
#define {macro_prefix}_SIZE {len(tflite_bytes)}{pcen_define}

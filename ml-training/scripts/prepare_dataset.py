...
def main():
    ...
    # Exclude quarantined tracks (unreliable labels — generated by correct_downbeats.py)
    quarantine_path = labels_dir / "quarantine.txt"
        ...
        with open(quarantine_path) as f:
            for line in f:
                ...
                if line and not line.startswith("#"):
                    quarantine_stems.add(line)
        before = len(pairs)
        pairs = [(a, l) for a, l in pairs if a.stem not in quarantine_stems]
        quarantined = before - len(pairs)
        if quarantined > 0:
            print(f"Quarantined {quarantined} tracks with unreliable labels "
                  f"(from {quarantine_path})")

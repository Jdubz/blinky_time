...
def firmware_mel_spectrogram(audio: np.ndarray, cfg: dict) -> np.ndarray:
    ...
    # Firmware applies soft-knee compressor to magnitudes before mel bands.
    # For training, we skip the compressor — it's an adaptive dynamic range
    # processor that depends on running state. The log compression below
    # provides equivalent range reduction for the CNN to learn from.
    # TODO: If parity issues arise, add a static approximation of the compressor.
...
def make_beat_targets(beat_times: np.ndarray, n_frames: int, frame_rate: float,
    ...
    for bt in beat_times:
        frame_idx = bt * frame_rate
        # Apply Gaussian centered at the beat
...
def process_file(audio_path: Path, label_path: Path, cfg: dict,
    ...
    Returns list of dicts with 'mel' and 'target' arrays.
    """
    sr = cfg["audio"]["sample_rate"]
...
def chunk_data(mel: np.ndarray, target: np.ndarray,
               chunk_frames: int, chunk_stride: int) -> tuple[np.ndarray, np.ndarray]:
    ...
    return np.array(chunks_mel), np.array(chunks_target)
...
def main():
    ...
    for audio_path, label_path in tqdm(pairs, desc="Processing"):
        try:
            ...
            for r in results:
                mel_chunks, target_chunks = chunk_data(

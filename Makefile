...
compile: check-arduino-cli update-device-type version
	...
	@echo "Board: $(FQBN)"
	@cd $(SKETCH_DIR) && $(ARDUINO_CLI) compile --fqbn $(FQBN) $(if $(EXTRA_CPP_FLAGS),--build-property "compiler.cpp.extra_flags=$(EXTRA_CPP_FLAGS)") .
...
# Update version from BUILD_NUMBER (prefer scripts/build.sh for auto-increment)
.PHONY: version
version:
	@BUILD=$$(cat $(SKETCH_DIR)/BUILD_NUMBER | tr -d '[:space:]'); \
	printf '#pragma once\n\n// Generated by Makefile version target.\n// See blinky-things/BUILD_NUMBER for the current value.\n\n#define FIRMWARE_BUILD %s\n#define FIRMWARE_VERSION "b%s"\n#define FIRMWARE_BUILD_DATE __DATE__ " " __TIME__\n' \
		"$$BUILD" "$$BUILD" > $(SKETCH_DIR)/types/Version.h; \
...
compile-out: safety-check check-arduino-cli update-device-type version
	...
	@echo "Sketch: $(SKETCH_DIR)"
	@echo "Board: $(FQBN)"
	@$(ARDUINO_CLI) compile --fqbn $(FQBN) $(if $(EXTRA_CPP_FLAGS),--build-property "compiler.cpp.extra_flags=$(EXTRA_CPP_FLAGS)") --output-dir $(BUILD_OUTPUT_DIR) $(SKETCH_DIR)/
	@echo "Compilation successful!"
...
BOOTLOADER_UF2 = bootloader/update-bootloader-qspi-ota-default.uf2

...
/**
 * WaterParticleParams - Water-specific particle parameters
 */
struct WaterParticleParams {
    // Spawn behavior
...
/**
 * WaterParticle - Particle-based water generator
 *
 * Features:
 ...
 */
class WaterParticle : public ParticleGenerator<64> {
public:
    WaterParticle();
    virtual ~WaterParticle() override = default;

    // Generator interface
    bool begin(const DeviceConfig& config) override;
    const char* getName() const override { return "WaterParticle"; }
    GeneratorType getType() const override { return GeneratorType::WATER; }

    // Parameter access
    void setParams(const WaterParticleParams& params) { params_ = params; }
    const WaterParticleParams& getParams() const { return params_; }
    WaterParticleParams& getParamsMutable() { return params_; }

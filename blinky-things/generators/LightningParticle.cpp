...
LightningParticle::LightningParticle() : params_() {}

bool LightningParticle::begin(const DeviceConfig& config) {
...
void LightningParticle::spawnParticles(float dt) {
...
void LightningParticle::updateParticle(Particle* p, float dt) {
...
void LightningParticle::renderParticle(const Particle* p, PixelMatrix& matrix) {
    int x = (int)p->x;
    int y = (int)p->y;
...
uint32_t LightningParticle::particleColor(uint8_t intensity) const {
    // Use Lightning palette
    return Palette::LIGHTNING.toColor(intensity);
}

void LightningParticle::spawnBranch(const Particle* parent) {

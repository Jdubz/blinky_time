...
/**
 * LightningParticleParams - Lightning-specific particle parameters
 */
struct LightningParticleParams {
    // Spawn behavior
...
/**
 * LightningParticle - Particle-based lightning generator
 *
 * Features:
 ...
 */
class LightningParticle : public ParticleGenerator<32> {
public:
    LightningParticle();
    virtual ~LightningParticle() override = default;

    // Generator interface
    bool begin(const DeviceConfig& config) override;
    const char* getName() const override { return "LightningParticle"; }
    GeneratorType getType() const override { return GeneratorType::LIGHTNING; }

    // Parameter access
    void setParams(const LightningParticleParams& params) { params_ = params; }
    const LightningParticleParams& getParams() const { return params_; }
    LightningParticleParams& getParamsMutable() { return params_; }
...
private:
    ...
    LightningParticleParams params_;
};

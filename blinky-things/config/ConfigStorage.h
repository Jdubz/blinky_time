...
public:
    ...
    static_assert(sizeof(StoredMusicParams) == 248,
        "StoredMusicParams size changed! Increment SETTINGS_VERSION and update assertion. (248 bytes = 52 floats + 8 uint8 + 19 bools + padding)");
    ...
    static_assert(sizeof(ConfigData) <= 660,
        "ConfigData exceeds 660 bytes! Current estimate ~653B. Check for unintended struct growth.");
